...
$(function () {
    ...
    if ($('.picture_menu').length > 0) {
        ...
        $('.picture_menu a').each(function() {
            $(this).click(function() {
                _gaq.push(['_trackEvent', 'SiteUsage', 'smallPreview', $(this).text()]);
                if ($(this).hasClass('buy_link')) {
                    var buy_template_id = $(this).attr('id').split('_')[1];
                    buy(buy_template_id);
                }
            });
        });
    ...
    var accountsPopup = new function() {
        ...
        this.init = function() {
            ...
            $('#sign_in,#sign_in_button,#edit_billing_details').click(function() {
                ...
                showWacPopup($(this).attr('href'), iframe_width, iframe_height, class_name);
                return false;
            }).removeAttr('onclick');
    ...
    if ($('#price_options').length > 0) {
        ...
        if (preview_offer_discount_labels.length > 0) {
            ...
            preview_offer_discount_labels.each(function(index) {
                discount_label_title_text = $(this).attr('title');
   ...
   /* --- Price options -> live demo page (end) --- */
    ...
    if ($('#networkBar').length > 0) {
        ...
        function hidePopup(popUp,popUpArrow) {
            popUp.slideUp(300, function() {popUpArrow.removeClass('active');});
        }

        var popUp = $('#networkBarPopup');
        var popUpArrow = $('#networkBarButtonArrow');

        $('#networkBarButton').bind('mouseenter', function() {if (!popUp.is(':visible')) {showPopup(popUp,popUpArrow);}});
        $('#networkBar').bind('mouseleave', function() {if (popUp.is(':visible')) {hidePopup(popUp,popUpArrow);}});
        ...
        /* --- Cart - Common (begin) --- */
            ...
            $('strike.fullPrice').live('mouseenter', function() {
                if ($(this).data('id')=='' || $(this).data('id')==undefined) {
                    $(this).data('id',$(this).attr('title')).removeAttr('title');
                }
        ...
        /* --- Cart - Step 2 (begin) --- */
            ...
            if ($('body').hasClass('second-step')) {
                ...
                $('#billing-email').blur(function() {
                    verify_email_info(page_meta.session_name + '=' + page_meta.session_ID + '&step=' + page_meta.cart_step + '&checkField=email', (trim(this.value) != ''));
                });
...
{
	var banner = $('#'+bannerId);
	banner.click(bannerFile,onclikGAHandler);
	banner.mouseup(bannerFile,onmouseupGAHandler);
}

function onclikGAHandler(e) {
		ga_track_banner(e.data)
}
